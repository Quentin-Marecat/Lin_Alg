...
SUBROUTINE QRHOUSE_HESS(DIM,MATRIX,Q,R,CHECK)
    ! --- FOR ANY REPORT OR SUGGESTION, PLEASE CONTACT quentin.marecat@etu.umontpellier.fr --- !
    ! -------------------------------------------------------------------------- !
    ! --- THIS SUBROUTINE GENERATE QR DECOMPOSITION USING HOUSEHOLDER SCHEME --- !
    ! --- MATRIX MUST BE HESSENBERG MATRIX ------------------------------------- !
    ...
    REAL*8, INTENT(IN) :: MATRIX(DIM,DIM)
    REAL*8, INTENT(OUT) :: R(DIM,DIM),Q(DIM,DIM)
    REAL*8 :: VEC(DIM),MATROTTAMP(DIM,DIM)
    REAL*8 :: MATTAMP(DIM,DIM)
    INTEGER :: I,J, LOWESTELEM, ERR, STAT
    ERR = 97
    OPEN(UNIT = ERR, FILE = 'error', IOSTAT = STAT, STATUS = 'old')
    IF (STAT == 0) CLOSE(ERR,STATUS = 'delete')
...
!        CALL PRODMAT(DIM,Q,MATROTTAMP,Q)
    ...
    IF (CHECK) THEN
        ...
        IF (TEST) THEN
            OPEN(UNIT = ERR,FILE = 'error')
            WRITE(ERR,'(A)') 'ERROR QR DECOMPOSITION HOUSEHOLDER HESSENBERG MATRIX'
            WRITE(ERR,'(A,4X,ES14.5)') 'CRIT ACPT',EPS
            WRITE(ERR,'(A)') 'Q ='
            DO I = 1,DIM
                WRITE(ERR,'(100F14.5)') (Q(I,J), J = 1,DIM)
